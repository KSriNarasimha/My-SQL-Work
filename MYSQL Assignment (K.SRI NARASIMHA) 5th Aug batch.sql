-- MYSQL Assignment (K.SRI NARASIMHA) -- August 5TH Batch
...
-- 1a.Fetch the employee number, first name and last name of those employees who are working as Sales Rep reporting to employee with employeenumber 1102 (Refer employee table)
...
-- 3b).	Company wants to analyse payment frequency by month. 
...
Count(Checknumber) as Number_of_payments
FROM Payments
Group By DATE_FORMAT(paymentDate, '%M')
Having Count(Checknumber) > 20
...
CREATE TABLE Customers (
    customer_id INT AUTO_INCREMENT PRIMARY KEY,     -- customer_id as PRIMARY KEY with AUTO_INCREMENT
    first_name VARCHAR(50) NOT NULL,                -- first_name with NOT NULL constraint
    last_name VARCHAR(50) NOT NULL,                 -- last_name with NOT NULL constraint
    email VARCHAR(255) UNIQUE,                      -- email as UNIQUE to ensure no duplicates
    phone_number VARCHAR(20)                        -- phone_number with no constraints, allows various formats
);
USE Customers_Orders;
select * from customers;
...
CREATE TABLE Orders (
    order_id INT AUTO_INCREMENT PRIMARY KEY,       -- order_id as PRIMARY KEY with AUTO_INCREMENT
    customer_id INT,                               -- customer_id column
    order_date DATE,                               -- order_date to store the date of the order
    total_amount DECIMAL(10,2),                    -- total_amount with 2 decimal places
    CONSTRAINT fk_customer FOREIGN KEY (customer_id) REFERENCES Customers(customer_id), -- FOREIGN KEY constraint
    CONSTRAINT chk_total_amount CHECK (total_amount > 0) -- CHECK constraint for positive total_amount
);
USE Customers_Orders;
select * from orders;
...
insert into project_Q6 values ( 1 , "Pranaya" , "Male" , 3) , 
                              (2 , "Priyanka", "Female", 1),
...
Select * from project_Q6;
commit;

-- Finding out the names of employees and their related managers.
                              
SELECT  m.Full_Name AS Manager_Name , e.Full_Name AS Emp_Name
from project_Q6 e join project_Q6 m 
...
create table Q7  (Facility_ID int, Name varchar(100), state varchar(100), country varchar(100));
...
commit;
select * from Q7;
-- See the below "describe Q7 " for the expected output reslut
Describe Q7;
...
-- : The category name of the product (from the ProductLines table).
...
SELECT
    pl.productLine,  -- Product category name from ProductLines table
    SUM(od.quantityOrdered * od.priceEach) AS total_sales,  -- Total revenue for the product category
    COUNT(DISTINCT o.orderNumber) AS number_of_orders  -- Total number of unique orders for the category
FROM
    ProductLines pl
JOIN
    Products p ON pl.productLine = p.productLine  -- Join ProductLines and Products to get the product category
JOIN
    OrderDetails od ON p.productCode = od.productCode  -- Join Products and OrderDetails to get sales data
JOIN
    Orders o ON od.orderNumber = o.orderNumber  -- Join OrderDetails and Orders to count unique orders
GROUP BY
    pl.productLine;  -- Group by product category to calculate total sales and orders per category
...
BEGIN
    -- Declare a variable to store the total payments
    DECLARE total_amount DECIMAL(10, 2);

    -- Calculate the total payments for the given year and country
    SELECT ROUND(SUM(p.amount) / 1000, 2) INTO total_amount
    FROM Payments p
    ...
    SELECT CONCAT(total_amount, 'K') AS formatted_total_amount;
END$$
...
WITH YearMonthOrders AS (
    SELECT
        EXTRACT(YEAR FROM orderDate) AS order_year,                  -- Extract the year from orderDate
        DATE_FORMAT(orderDate, '%M') AS month_name,                   -- Get the full month name
        COUNT(*) AS order_count                                       -- Count the number of orders
    FROM
        Orders
    GROUP BY
        EXTRACT(YEAR FROM orderDate),                                -- Group by year
        DATE_FORMAT(orderDate, '%M')                                 -- Group by month name
),
YoY_PercentageChange AS (
    SELECT
        order_year,
        month_name,
        order_count,
        -- Get the order count of the same month from the previous year using LAG
        Lag(order_count) OVER ( ORDER BY order_year) AS prev_year_count
    FROM
        YearMonthOrders
)
SELECT
    ...
    month_name,
    order_count,
    -- Calculate the YoY percentage change and format it as requested
    CASE
        WHEN prev_year_count IS NULL THEN NULL  -- Handle first year, no previous year data
        ELSE CONCAT(
            ROUND(((order_count - prev_year_count) * 100 / prev_year_count), 0),  -- YoY Change Formula
...
ORDER BY
    order_year,
    FIELD(month_name, 'January', 'February', 'March', 'April', 'May', 'June',  -- Ensure correct month order
...
-- Execute "InserEmployee" stored procedure for the employee details insertion and error message for the dublicate records entered
-- We get "successfully recorded message for the correct data insertion
-- We get "Error Occured " message for the Dublicate data insertion or incorrect data insertion
...
-- I have created Before insert trigger for converting negative working_hours to the positive working_hours using ABS function
select * from emp_BIT;
-- Now , i found it is successfully converted negative working_hours to the positive using "Before insert trigger" as asked in the assigment.
